...
class Shop_Model extends TinyMVC_Model
{
	...
	{
		$this->db->query('select * from item_template where type' . $cat . ' and publier = 1 ORDER BY item_template.level DESC');
		$array_shop = array();
		$i          = 0;
		while ($row = $this->db->next()) {
			...
			/* Ligne qui permet de recup le nom de la panoplie (bug)
			$array_shop[$i]['panoplie'] = self::get_panoplie_name($row['panoplie']);
			*/
			$array_shop[$i]['panoplie'] = $row['panoplie'];
			$array_shop[$i]['stats'] = self::statsitem($row['statsTemplate'], FALSE, 'oui', 'non');
			$i++;
		} //$row = $this->db->next()
		return $array_shop;
	}
	...
	{
		$name = $this->db->query_one('select * from item_template where ID=?', array(
			$id
		));
		return $name['name'];
	}
	...
	function shop_add_log($type, $username, $item_b, $date)
	{
	...
	function get_shop_log()
	{
		$array_shop_log = array();
		$this->db->query('select * from panty_log WHERE type = 2 ORDER by Date DESC LIMIT 50');
		$i          = 1;
		while ($row = $this->db->next()) {
			$array_shop_log[$i]['account'] = $row['account'];
			$array_shop_log[$i]['item'] = $row['item'];
			$array_shop_log[$i]['date']    = $row['date'];
			$i++;
		} //$row = $this->db->next()
		return $array_shop_log;
	}
}