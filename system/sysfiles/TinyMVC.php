...
set_include_path(get_include_path() . PATH_SEPARATOR . TMVC_MYAPPDIR . 'controllers' . DS . PATH_SEPARATOR . TMVC_MYAPPDIR . 'models' . DS . PATH_SEPARATOR . TMVC_MYAPPDIR . 'configs' . DS . PATH_SEPARATOR . TMVC_MYAPPDIR . 'plugins' . DS . PATH_SEPARATOR . TMVC_MYAPPDIR . 'views' . DS
...
//. PATH_SEPARATOR . TMVC_BASEDIR . 'sysfiles' . DS . 'configs' . DS
	. PATH_SEPARATOR . TMVC_BASEDIR . 'sysfiles' . DS . 'plugins' . DS . PATH_SEPARATOR . TMVC_BASEDIR . 'sysfiles' . DS . 'views' . DS);
...
class tmvc
{
	...
	public function setupErrorHandling()
	{
		...
		} //defined('TMVC_ERROR_HANDLING') && TMVC_ERROR_HANDLING == 1
	}
	/**
	...
	public function setupController()
	{
		...
		if (!empty($this->config['root_controller'])) {
		...
		} //!empty($this->config['root_controller'])
		else {
			...
			if (!stream_resolve_include_path($controller_file)) {
				...
				$controller_file = "{$controller_name}.php";
			} //!stream_resolve_include_path($controller_file)
		}
