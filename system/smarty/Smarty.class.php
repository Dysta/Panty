...
} //!defined('SMARTY_RESOURCE_CHAR_SET')
if (!defined('SMARTY_RESOURCE_DATE_FORMAT')) {
	/**
	 * @deprecated in favor of Smarty::$_DATE_FORMAT
	 */
	define('SMARTY_RESOURCE_DATE_FORMAT', '%b %e, %Y');
} //!defined('SMARTY_RESOURCE_DATE_FORMAT')
/**
 * register the class autoloader
 */
if (!defined('SMARTY_SPL_AUTOLOAD')) {
	define('SMARTY_SPL_AUTOLOAD', 0);
} //!defined('SMARTY_SPL_AUTOLOAD')
if (SMARTY_SPL_AUTOLOAD && set_include_path(get_include_path() . PATH_SEPARATOR . SMARTY_SYSPLUGINS_DIR) !== false) {
	$registeredAutoLoadFunctions = spl_autoload_functions();
	if (!isset($registeredAutoLoadFunctions['spl_autoload'])) {
		spl_autoload_register();
	} //!isset($registeredAutoLoadFunctions['spl_autoload'])
} //SMARTY_SPL_AUTOLOAD && set_include_path(get_include_path() . PATH_SEPARATOR . SMARTY_SYSPLUGINS_DIR) !== false
else {
...
class Smarty extends Smarty_Internal_TemplateBase
{
	...
	public function __construct()
	{
		...
		} //is_callable('mb_internal_encoding')
	...
	public function getGlobal($varname = null)
	{
		if (isset($varname)) {
			...
			} //isset(self::$global_tpl_vars[$varname])
			else {
				return '';
			}
		} //isset($varname)
			...
			} //self::$global_tpl_vars AS $key => $var
			return $_result;
		}
	}
	/**
	...
	public function enableSecurity($security_class = null)
	{
		if ($security_class instanceof Smarty_Security) {
			...
			return $this;
		} //$security_class instanceof Smarty_Security
		elseif (is_object($security_class)) {
			throw new SmartyException("Class '" . get_class($security_class) . "' must extend Smarty_Security.");
		} //is_object($security_class)
		if ($security_class == null) {
			$security_class = $this->security_class;
		} //$security_class == null
		if (!class_exists($security_class)) {
			throw new SmartyException("Security class '$security_class' is not defined");
		} //!class_exists($security_class)
		elseif ($security_class !== 'Smarty_Security' && !is_subclass_of($security_class, 'Smarty_Security')) {
			throw new SmartyException("Class '$security_class' must extend Smarty_Security.");
		} //$security_class !== 'Smarty_Security' && !is_subclass_of($security_class, 'Smarty_Security')
		else {
	...
	public function addAutoloadFilters($filters, $type = null)
	{
		if ($type !== null) {
			...
			} //!empty($this->autoload_filters[$type])
			else {
				$this->autoload_filters[$type] = (array) $filters;
			}
		} //$type !== null
	...
	public function createTemplate($template, $cache_id = null, $compile_id = null, $parent = null, $do_clone = true)
	{
		if (!empty($cache_id) && (is_object($cache_id) || is_array($cache_id))) {
			$parent   = $cache_id;
			$cache_id = null;
		} //!empty($cache_id) && (is_object($cache_id) || is_array($cache_id))
		...
		if ($do_clone) {
			...
			} //isset($this->template_objects[$_templateId])
			else {
	...
	public function loadPlugin($plugin_name, $check = true)
	{
		...
		if ($check && (is_callable($plugin_name) || class_exists($plugin_name, false))) {
			return true;
		} //$check && (is_callable($plugin_name) || class_exists($plugin_name, false))
		...
		if (!isset($_name_parts[2]) || strtolower($_name_parts[0]) !== 'smarty') {
			...
			return false;
		} //!isset($_name_parts[2]) || strtolower($_name_parts[0]) !== 'smarty'
		...
		} //strtolower($_name_parts[1]) == 'internal'
		...
		foreach ($this->getPluginsDir() as $_plugin_dir) {
			...
			foreach ($names as $file) {
				...
				} //$this->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $_plugin_dir)
			} //$names as $file
		} //$this->getPluginsDir() as $_plugin_dir
		// no plugin loaded
		return false;
	}
	/**
	...
	public static function mutingErrorHandler($errno, $errstr, $errfile, $errline, $errcontext)
	{
		...
		} //!isset(Smarty::$_muted_directories[SMARTY_DIR])
		...
		foreach (Smarty::$_muted_directories as $key => &$dir) {
			...
			if (!strncmp($errfile, $dir['file'], $dir['length'])) {
				...
				break;
			} //!strncmp($errfile, $dir['file'], $dir['length'])
		} //Smarty::$_muted_directories as $key => &$dir
		...
		} //!$_is_muted_directory || ($errno && $errno & error_reporting())
	}
	/**
...
function smartyAutoload($class)
{
	...
	} //!strncmp($_class, 'smarty_internal_', 16) || isset($_classes[$_class])
}