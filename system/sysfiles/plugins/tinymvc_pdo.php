...
class TinyMVC_PDO
{
	...
	private function _assemble_where(&$where, &$params)
	{
		if (!empty($this->query_params['where'])) {
			...
			foreach ($this->query_params['where'] as $cwhere) {
				...
				$where_init    = true;
			} //$this->query_params['where'] as $cwhere
			$where = implode(' ', $where_parts);
			return true;
		} //!empty($this->query_params['where'])
		return false;
	}
	/**
	 * query
	...
	function update($table, $columns)
	{
		...
		if (empty($columns) || !is_array($columns)) {
			...
			return false;
		} //empty($columns) || !is_array($columns)
		$query  = array(
			"UPDATE {$table} SET"
		);
		$fields = array();
		...
		} //$columns as $cname => $cvalue
		$query[] = implode(',', $fields);
		// assemble where clause
		if ($this->_assemble_where($where_string, $where_params)) {
			$query[] = $where_string;
			$params  = array_merge($params, $where_params);
		} //$this->_assemble_where($where_string, $where_params)
		$query              = implode(' ', $query);
		$this->query_params = array(
			'select' => '*'
		);
